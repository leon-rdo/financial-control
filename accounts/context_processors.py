...
def urls_context(request):
    ...
    for link in urls_data:
        if "permissions" in link and check_permission(request.user, link["permissions"]):
            ...
            for sub_link in processed_link.get("sub_urls", []):
                if "permissions" in sub_link and check_permission(request.user, sub_link["permissions"]):
